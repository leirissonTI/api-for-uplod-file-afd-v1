...
export class EscalaService {
     ...
     */
    async getAllEscalas(): Promise<EscalaEntity[]> {
        try {
    ...
    async createEscalaServidor(data: TCreateEscalaDto) {
        ...
        if (diasEscala && diasEscala.length > 0) {
            for (const d of diasEscala) {
                const v = d instanceof Date ? d : new Date(d as any)
                if (!(v instanceof Date) || isNaN(v.getTime())) throw new Error('diasEscala contém data inválida')
                datas.push(v)
            }
        } else if (dataEscala) {
