...
export class EscalaService {
    ...
    async createEscalaServidor(data: TCreateEscalaDto) {
        ...
        let servidorNome: string | undefined
        ...
        let chefeNome: string | undefined
        ...
        let cpfServidor: string | undefined
        ...
        for (const item of datasOpcao) {
            ...
            try {
                ...
                if (criadorIdFinal) {
                    ...
                    let espelhoDiarioId: number | undefined
                    ...
                    await this.prismaService.solicitacao.create({
                        data: {
                            ...
                            opcao: (() => {
                                const iso = dt.toISOString().slice(0,10)
                                if (setPagamento.has(iso)) return 'PAGAMENTO' as any
                                if (setFolga.has(iso)) return 'FOLGA' as any
                                if (opcao === 'pagamento') return 'PAGAMENTO' as any
                                if (opcao === 'folga') return 'FOLGA' as any
                                return (!!receberPagamento ? 'PAGAMENTO' : 'FOLGA') as any
                            })(),
                        }
                    })
