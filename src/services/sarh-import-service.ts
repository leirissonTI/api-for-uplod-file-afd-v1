...
type TSarhRow = {
  ...
  CIDADE?: string
  TELEFONE?: string
  ID_TIPO_LOTACAO?: string
  LOTACAO?: string
  SIGLA?: string
  CARGO?: string
}

export class SarhImportService {
  ...
  private parseCsv(content: string): TSarhRow[] {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      rows.push(row as TSarhRow)
    }
    return rows
  }

  private clampString(v: any, max: number): string | null {
  ...
  private toIntSafe(v: any, max?: number): number | null {
  ...
  private async existsByCod(cod: number): Promise<boolean> {
    const r = await this.prismaService.$queryRawUnsafe<any[]>(`SELECT 1 FROM "sarh_funcionario" WHERE "COD_FUNCIONARIO" = ${cod} LIMIT 1`)
    return Array.isArray(r) && r.length > 0
  }

  async importFromCsv(filePath: string): Promise<{ inserted: number; updated: number; skipped: number; errors: string[] }> {
    ...
    let startId = 0
    try {
      const res: Array<{ max?: number }> = await this.prismaService.$queryRawUnsafe(`SELECT COALESCE(MAX("COD_FUNCIONARIO"),0) AS max FROM "sarh_funcionario"`)
    ...
    for (const r of items) {
      try {
        ...
        const existentMat: Array<{ COD_FUNCIONARIO?: number }> = await this.prismaService.$queryRawUnsafe(`SELECT "COD_FUNCIONARIO" FROM "sarh_funcionario" WHERE "MATRICULA" = '${matricula}' LIMIT 1`)
        const codCsv = this.toIntSafe(r.COD_FUNCIONARIO, 2147483647)
        const existentCod: Array<{ COD_FUNCIONARIO?: number }> = codCsv != null ? await this.prismaService.$queryRawUnsafe(`SELECT "COD_FUNCIONARIO" FROM "sarh_funcionario" WHERE "COD_FUNCIONARIO" = ${codCsv} LIMIT 1`) : []
        const payload = {
        ...
        const colsAll = Object.keys(payload).filter(k => (payload as any)[k] !== undefined)
        const valueExpr = (col: string, val: any) => {
          if (val === null || val === undefined || val === '') return 'NULL'
          if (numericCols.has(col)) return String(val)
        ...
        if ((existentMat && existentMat.length) || (existentCod && existentCod.length)) {
          ...
          const setExpr = updCols.map(c => `"${c}" = ${valueExpr(c, (payload as any)[c])}`).join(', ')
          ...
          if (!payload.COD_FUNCIONARIO) {
            ...
            if (existentCod && existentCod.length) {
              do { nextId += 1 } while (await this.existsByCod(nextId))
              payload.COD_FUNCIONARIO = nextId
            }
          }
          const insCols = Object.keys(payload).filter(k => (payload as any)[k] !== undefined)
          const insVals = insCols.map(c => valueExpr(c, (payload as any)[c]))
  ...
  async listFuncionarios(params: { q?: string; limit?: number; offset?: number }) {
