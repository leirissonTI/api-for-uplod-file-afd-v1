...
exports.EspelhoPontoService = void 0;
const prisma_1 = require("../config/prisma");
const app_erro_1 = require("../utils/app-erro");
const getDiasUteis_1 = require("../utils/getDiasUteis");
...
class EspelhoPontoService {
    ...
    async getEspelhoDiarioPorMes(cpf, inicioDoMes) {
        try {
            ...
            const registros = await prisma_1.prisma.$queryRawUnsafe(sql);
            return registros;
        }
        catch (error) {
            throw new app_erro_1.AppError(`Erro ao tentar resgatar os pontos do mês, mensagem: ${error.message}`);
        }
    }
    async getEspelhomensal() {
        const espelhoMensal = prisma_1.prisma.espelhoMensal.findMany();
        return espelhoMensal;
    }
    async getEspelhoRecessoSummary(params) {
        const recesso = await prisma_1.prisma.recesso.findUnique({ where: { id: params.recessoId }, select: { DataInicial: true, dataFinal: true } });
        if (!recesso)
            throw new app_erro_1.AppError('Recesso não encontrado.');
        ...
        const sqlCpfs = `
          ...
          ${filtroMat}
        `;
        const pessoas = await prisma_1.prisma.$queryRawUnsafe(sqlCpfs);
        ...
        for (const p of pessoas) {
            if (!p.cpf)
                continue;
            const rows = await prisma_1.prisma.espelhoDiario.findMany({ where: { cpf: p.cpf, mesAno: { in: alvoMeses } }, select: { data: true, horasTrabalhadas: true, status: true } });
            const dentro = rows.map((r) => ({ data: toDate(String(r.data)), horas: Number(r.horasTrabalhadas || 0), status: String(r.status || '') })).filter((x) => x.data && x.data >= start && x.data <= end);
            const diasUteis = businessDaysBetween(start, end);
            const diasTrabalhados = dentro.filter((x) => x.status === 'Presente' || x.horas > 0).length;
            const faltas = Math.max(diasUteis - diasTrabalhados, 0);
            const totalHorasTrabalhadas = Number(dentro.reduce((a, b) => a + (isNaN(b.horas) ? 0 : b.horas), 0).toFixed(2));
    ...
    async getEspelhoRecessoDiario(params) {
        const recesso = await prisma_1.prisma.recesso.findUnique({ where: { id: params.recessoId }, select: { DataInicial: true, dataFinal: true } });
        if (!recesso)
            throw new app_erro_1.AppError('Recesso não encontrado.');
        const start = new Date(recesso.DataInicial);
    ...
    `;
        ...
        for (const p of pessoas) {
            if (!p.cpf)
                continue;
            const rows = await prisma_1.prisma.espelhoDiario.findMany({ where: { cpf: p.cpf, mesAno: { in: alvoMeses } }, select: { data: true, mesAno: true, horasTrabalhadas: true, status: true } });
            for (const r of rows) {
    ...
    async getEspelhoFormado(cpf, mes, ano) {
        ...
        if (!validarCPF(cpf)) {
            throw new app_erro_1.AppError('CPF inválido.');
        }
        const mesAno = formatarMesAno(mes, ano);
        try {
            const registros = await prisma_1.prisma.$queryRaw(prisma_2.Prisma.sql `
            ...
            throw new app_erro_1.AppError('Erro ao tentar resgatar os pontos do mês.', error);
        }
    }
    async gerarEspelhoMensal(cpf, inicioDoMes, inicioDoProximoMes) {
        try {
            // Busca de registros do banco
            const registros = await prisma_1.prisma.marcacoesRelogio.findMany({
                where: {
                    cpfEmpregado: cpf,
                    dataCompleta: {
                        gte: inicioDoMes,
            ...
            for (const data of todasAsDatas) {
                ...
                if (marcas.length > 0) {
                    ...
                    espelhoDiario = this.processarDia(cpf, mesAno, dataStr, marcas);
                }
                else {
            ...
            await prisma_1.prisma.$transaction(async (tx) => {
                ...
                const totalHorasTrabalhadas = espelhosDiarios.reduce((acc, curr) => {
                    const horas = (typeof curr.horasTrabalhadas === 'string')
                        ? parseFloat(curr.horasTrabalhadas.replace(',', '.'))
    ...
    gerarTodasDatasDoMes(dataInicial) {
        const datas = [];
 ...
 */
    converterMinutosParaHorasHMM(minutos) {
        if (minutos === 0)
            return 0;
        if (!isFinite(minutos))
            return 0;
    ...
    processarDia(cpf, mesAno, dataStr, marcas) {
        ...
        const observacoes = [];
        if (!primeiraEntrada)
            observacoes.push('Primeira entrada não registrada');
        if (!primeiraSaida)
            observacoes.push('Primeira saída não registrada');
        if (!segundaEntrada)
            observacoes.push('Segunda entrada não registrada');
        if (!segundaSaida)
            observacoes.push('Segunda saída não registrada');
        if (observacaoDuplicadas)
            observacoes.push(observacaoDuplicadas);
        // Parse da data
...
exports.EspelhoPontoService = EspelhoPontoService;
